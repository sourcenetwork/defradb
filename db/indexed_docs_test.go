	...
	cases := []struct {
		...
		},
		{
			...
			PrepareFetcher: func() fetcher.Fetcher {
				...
				f.EXPECT().FetchNextDecoded(mock.Anything).Return(nil, fetcher.Stats{}, testError)
				f.EXPECT().Close().Unset()
				f.EXPECT().Close().Return(nil)
				return f
			},
		},
		{
			...
			PrepareFetcher: func() fetcher.Fetcher {
				f := fetcherMocks.NewStubbedFetcher(t)
				f.EXPECT().FetchNextDecoded(mock.Anything).Unset()
				f.EXPECT().FetchNextDecoded(mock.Anything).Return(nil, fetcher.Stats{}, nil)
				f.EXPECT().Close().Unset()
	...
	cases := []struct {
		...
		},
		{
			...
			PrepareFetcher: func() fetcher.Fetcher {
				...
				f.EXPECT().FetchNextDecoded(mock.Anything).Maybe().Return(nil, fetcher.Stats{}, nil)
				f.EXPECT().Close().Unset()
				f.EXPECT().Close().Return(testError)
				return f
			},
...
func TestNonUniqueUpdate_IfDatastoreFails_ReturnError(t *testing.T) {
	...
	for _, tc := range cases {
		...
		f.users.fetcherFactory = func() fetcher.Fetcher {
			...
			df.EXPECT().FetchNextDecoded(mock.Anything).Return(doc, fetcher.Stats{}, nil)
			return df
		}
