...
func Benchmark_Query_UserSimple_Query_WithMultiLookup_Sync_10(b *testing.B) {
	...
	err := runQueryBenchGet(b, ctx, fixtures.ForSchema(ctx, "user_simple"), 10, userSimpleWithMultiLookupQuery, false)
	if err != nil {
		b.Fatal(err)
	}
}
...
func Benchmark_Query_UserSimple_Query_WithMultiLookup_Sync_100(b *testing.B) {
	ctx := context.Background()
	err := runQueryBenchGet(b, ctx, fixtures.ForSchema(ctx, "user_simple"), 100, userSimpleWithMultiLookupQuery, false)
	if err != nil {
		b.Fatal(err)
	}
}
...
func Benchmark_Query_UserSimple_Query_WithMultiLookup_Sync_1000(b *testing.B) {
	ctx := context.Background()
	err := runQueryBenchGet(b, ctx, fixtures.ForSchema(ctx, "user_simple"), 1000, userSimpleWithMultiLookupQuery, false)
	if err != nil {
		b.Fatal(err)
	}
}
