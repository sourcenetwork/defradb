...
func DefaultACPOptions() *options.NodeDocumentACPOptions {
...
func NewDocumentACP(
	...
	if opt == nil {
		opt = DefaultACPOptions()
	}
	acpConstructor, ok := documentACPConstructors[opt.DocumentACPType]
	if ok {
		return acpConstructor(ctx, opt)
	}
	return immutable.None[dac.DocumentACP](), NewErrACPTypeNotSupported(opt.DocumentACPType)
}
