...
// drainMutationUpdateEvents drains the update events generated by a GQL mutation.
// Mutation operations (create, update, delete) fire update events via txn.OnSuccess that
// action.Request does not wait for. Without draining, these events accumulate in the channel
// and are received as unexpected events by the next action that calls waitForUpdateEvents.
func drainMutationUpdateEvents(s *state.State, nodeID immutable.Option[int], data any) {
	...
	for key, val := range dataMap {
		if !strings.HasPrefix(key, "delete_") && !strings.HasPrefix(key, "update_") && !strings.HasPrefix(key, "create_") {
			continue
		}
		switch v := val.(type) {
		case []any:
