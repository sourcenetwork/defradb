...
type randomDocGenerator struct {
	...
	config     map[string]map[string]genConfig
	...
	for _, typeName := range order {
		...
		for i := 0; i < count; i++ {
			...
			for _, field := range typeDef.fields {
				if field.isRelation {
					if field.isPrimary {
						relDocInd := incrementCounter(field.typeStr, typeName, field.name)
						docKey := getDocKey(field.typeStr, relDocInd)
						newDoc[field.name+request.RelatedObjectID] = docKey
					}
				} else {
					newDoc[field.name] = generateRandomValue(field.typeStr)
				}
			}
