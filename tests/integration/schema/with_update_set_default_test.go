...
func TestSchema_WithUpdateAndSetDefaultVersionToOriginal_NewFieldIsNotQueriable(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.SetDefaultSchemaVersion{
				SchemaVersionID: "bafkreih27vuxrj4j2tmxnibfm77wswa36xji74hwhq7deipj5rvh3qyabq",
			},
			testUtils.Request{
				Request: `query {
					Users {
...
func TestSchema_WithUpdateAndSetDefaultVersionToNew_AllowsQueryingOfNewField(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.SetDefaultSchemaVersion{
				SchemaVersionID: "bafkreid5bpw7sipm63l5gxxjrs34yrq2ur5xrzyseez5rnj3pvnvkaya6m",
			},
			testUtils.Request{
				Request: `query {
					Users {
