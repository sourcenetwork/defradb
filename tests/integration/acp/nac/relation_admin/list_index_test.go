...
func TestNAC_AdminRelation_CanListIndex(t *testing.T) {
	test := testUtils.TestCase{
		Description: "NAC admin relation grants a user permission to list indexes for a collection.",
...
func TestNAC_AdminRelation_CLIandCandHTTPClient_CanListIndex(t *testing.T) {
	test := testUtils.TestCase{
		Description: "NAC admin relation grants CLI, C and HTTP clients permission to list collection indexes.",
		// todo: Investigate and test this behavior across all client types when implementing granular NAC permissions.
		// See: https://github.com/sourcenetwork/defradb/issues/4383
		SupportedClientTypes: immutable.Some(
			[]state.ClientType{
