...
func TestNAC_AdminRelation_WithoutDACManagerRelation_CanNotDeleteDACActorRelationship(t *testing.T) {
	test := testUtils.TestCase{
		Actions: []any{
			...
			// But, since this user is not a DAC manager, they can't delete a DAC actor relationship.
			testUtils.DeleteDACActorRelationship{
				RequestorIdentity: testUtils.ClientIdentity(2),
				TargetIdentity:    testUtils.ClientIdentity(3),
				CollectionID:      0,
				DocID:             0,
				Relation:          "reader",
				ExpectedError:     "cannot delete relationship: actor is not a manager of relation",
			},
		},
	}
