...
func TestNAC_Disabled_WithDACEnabled_AccessByNodeOwner_DoesNotOwnTheDocument_CanNotAccess(t *testing.T) {
	test := testUtils.TestCase{
		Description: "With NAC disabled, the node owner identity cannot bypass DAC and cannot access a document they do not own.",
		Actions: []any{
...
func TestNAC_Disabled_WithDACEnabled_AccessByNodeOwner_OwnsTheDocument_CanAccess(t *testing.T) {
	test := testUtils.TestCase{
		Description: "With NAC disabled, the node owner can access their own private document via DAC.",
		Actions: []any{
			// Starting with NAC, so only authorized user(s) can perform operations from here on out.
			testUtils.Close{},
			testUtils.Start{
...
func TestNAC_Disabled_WithDACEnabled_AccessByNodeOwner_PublicDocument_CanAccess(t *testing.T) {
	test := testUtils.TestCase{
		Description: "With NAC disabled, the node owner can access public documents.",
		Actions: []any{
			// Starting with NAC, so only authorized user(s) can perform operations from here on out.
			testUtils.Close{},
			testUtils.Start{
