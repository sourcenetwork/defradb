...
func configureReplicator(
	...
	ident := getIdentityForRequestSpecificToNode(s, cfg.Identity, cfg.SourceNodeID)
	opt := options.WithIdentity(options.SetReplicator(), ident)
...
func deleteReplicator(
	...
	ctx := getContextWithIdentity(s.Ctx, s, cfg.Identity, cfg.SourceNodeID)
	ident := getIdentityForRequestSpecificToNode(s, cfg.Identity, cfg.SourceNodeID)
	opt := options.WithIdentity(options.DeleteReplicator(), ident)
