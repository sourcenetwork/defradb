...
// already satisfied and no order node is added. This is what keeps the scan lazy: an order node
// drains its whole source into a sort buffer on its first Next, and the limit below it would then
// never stop the scan early.
//
...
func TestBM25Query_OrderedByRankDescending_DropsTheOrderNode(t *testing.T) {
	test := testUtils.TestCase{
		Actions: bm25ArticleActions(`@index(kind: BM25)`, &action.ExplainRequest{
...
func TestBM25Query_WithFieldLengthNormalisation_ChangesTheOrder(t *testing.T) {
	...
	assert(`@index(kind: BM25, options: {b: 0})`, []string{"long", "short"})
	// With it on the longer document's two occurrences are worth less than the shorter one's one.
	assert(`@index(kind: BM25, options: {b: 0.75})`, []string{"short", "long"})
}
...
func TestBM25Query_WithTermFrequencySaturation_ChangesTheOrder(t *testing.T) {
	...
	assert(`@index(kind: BM25, options: {k1: 0})`, []string{"both terms", "repeated"})
	assert(`@index(kind: BM25, options: {k1: 10})`, []string{"repeated", "both terms"})
}
...
func TestBM25Query_AfterUpdateAndDelete_RanksWhatIsLeft(t *testing.T) {
	test := testUtils.TestCase{
		Actions: bm25ArticleActions(`@index(kind: BM25)`,
