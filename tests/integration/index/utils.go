...
func (a explainResultAsserter) Assert(t *testing.T, result []dataMap) {
	require.Len(t, result, 1, "Expected one result, got %d", len(result))
	explainNode, ok := result[0]["explain"].(dataMap)
	require.True(t, ok, "Expected explain")
	assert.Equal(t, explainNode["executionSuccess"], true)
	assert.Equal(t, explainNode["sizeOfResult"], 1)
	assert.Equal(t, explainNode["planExecutions"], uint64(2))
	...
	assert.Equal(t, scanNode["iterations"], uint64(a.iterations),
		"Expected %d iterations, got %d", a.iterations, scanNode["iterations"])
	assert.Equal(t, scanNode["docFetches"], uint64(a.docFetches),
		"Expected %d docFetches, got %d", a.docFetches, scanNode["docFetches"])
	assert.Equal(t, scanNode["filterMatches"], uint64(a.filterMatches),
		"Expected %d filterMatches, got %d", a.filterMatches, scanNode["filterMatches"])
}
