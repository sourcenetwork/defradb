...
func subscribeToCollection(
	...
	ident := getIdentityForRequestSpecificToNode(s, action.Identity, action.NodeID)
	opt := options.WithIdentity(options.AddP2PCollections(), ident)
...
func unsubscribeToCollection(
	...
	ctx := getContextWithIdentity(s.Ctx, s, action.Identity, action.NodeID)
	ident := getIdentityForRequestSpecificToNode(s, action.Identity, action.NodeID)
	opt := options.WithIdentity(options.RemoveP2PCollections(), ident)
...
func getAllP2PCollections(
	...
	ctx := getContextWithIdentity(s.Ctx, s, action.Identity, action.NodeID)
	ident := getIdentityForRequestSpecificToNode(s, action.Identity, action.NodeID)
	opt := options.WithIdentity(options.GetAllP2PCollections(), ident)
