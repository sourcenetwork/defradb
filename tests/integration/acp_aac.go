...
type GetAACStatus struct {
	...
	// ExpectedIsEnabled returns true if admin acp is currently enabled, and false otherwise.
	ExpectedIsEnabled bool

	// ExpectedIsConfigured returns true if admin acp was ever configured, and false otherwise.
	ExpectedIsConfigured bool
...
func getAACStatus(
	...
	for index, node := range nodes {
		...
		if !expectedErrorRaised {
			require.Equal(s.t, action.ExpectedError, "")
			require.Equal(s.t, action.ExpectedIsEnabled, statusAACResult.IsEnabled)
			require.Equal(s.t, action.ExpectedIsConfigured, statusAACResult.IsConfigured)
		}
	}
}
