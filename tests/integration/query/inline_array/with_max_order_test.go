...
func TestQueryInlineIntegerArray_WithMaxAndOrder_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "MAX across multiple inline arrays can be used as an order key in ASC and DESC queries.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"testScores": [3, 4, 5],
...
func TestQueryInlineIntegerArray_WithNullAndMaxAndOrder_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "MAX across nillable inline arrays with null elements can be used as an order key.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
