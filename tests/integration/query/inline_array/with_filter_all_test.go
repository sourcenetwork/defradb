// Copyright 2026 Democratized Data Foundation
//
// This file is part of the DefraDB test suite.
//
// The DefraDB test suite is licensed under either:
//
//   (1) GNU Affero General Public License v3
//   (2) Business Source License 1.1
//
// See tests/LICENSE for details.

package inline_array

import (
	"testing"

	"github.com/sourcenetwork/defradb/tests/action"
	testUtils "github.com/sourcenetwork/defradb/tests/integration"
)

func TestQueryInlineStringArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a nillable string array returns only docs where all elements match.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"pageHeaders": ["first", "second"]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"pageHeaders": [null, "second"]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {pageHeaders: {_all: {_neq: null}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Shahzad",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineNotNullStringArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a non-null string array returns only docs where every element is non-empty.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"preferredStrings": ["first", "second"]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"preferredStrings": ["", "second"]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {preferredStrings: {_all: {_neq: ""}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Shahzad",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineIntArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a nillable integer array returns only docs where no elements are null.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"testScores": [50, 80]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"testScores": [null, 60]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {testScores: {_all: {_neq: null}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Shahzad",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineNotNullIntArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a non-null integer array returns docs where all elements satisfy the condition.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"testScores": [50, 80]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"testScores": [0, 60]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {testScores: {_all: {_lt: 70}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Fred",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineFloatArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a nillable float array returns only docs where no elements are null.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"pageRatings": [50, 80]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"pageRatings": [null, 60]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {pageRatings: {_all: {_neq: null}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Shahzad",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineNotNullFloatArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a non-null float array returns docs where all elements satisfy the condition.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"pageRatings": [50, 80]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"pageRatings": [0, 60]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {pageRatings: {_all: {_lt: 70}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Fred",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineBooleanArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a nillable boolean array returns only docs where no elements are null.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"indexLikesDislikes": [false, false]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"indexLikesDislikes": [null, true]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {indexLikesDislikes: {_all: {_neq: null}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Shahzad",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineNotNullBooleanArray_WithAllFilter_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a non-null boolean array returns docs where all elements are true.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Shahzad",
					"likedIndexes": [false, false]
				}`,
			},
			&action.AddDoc{
				Doc: `{
					"name": "Fred",
					"likedIndexes": [true, true]
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {likedIndexes: {_all: {_eq: true}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{
						{
							"name": "Fred",
						},
					},
				},
			},
		},
	}

	executeTestCase(t, test)
}

func TestQueryInlineStringArray_WithAllFilterAndNullValue_Succeeds(t *testing.T) {
	test := testUtils.TestCase{
		Description: "Filtering with _all on a null inline string array returns no documents.",
		Actions: []any{
			&action.AddDoc{
				Doc: `{
					"name": "Islam",
					"pageHeaders": null
				}`,
			},
			&action.Request{
				Request: `query {
					Users(filter: {pageHeaders: {_all: {_eq: null}}}) {
						name
					}
				}`,
				Results: map[string]any{
					"Users": []map[string]any{},
				},
			},
		},
	}

	executeTestCase(t, test)
}
