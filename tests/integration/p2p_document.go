...
func subscribeToDocument(
	...
	opt := options.WithIdentity(options.AddP2PDocuments(), ident)
...
func unsubscribeToDocument(
	...
	ctx := getContextWithIdentity(s.Ctx, s, action.Identity, action.NodeID)
	ident := getIdentityForRequestSpecificToNode(s, action.Identity, action.NodeID)
	opt := options.WithIdentity(options.RemoveP2PDocuments(), ident)
...
func getAllP2PDocuments(
	...
	ctx := getContextWithIdentity(s.Ctx, s, action.Identity, action.NodeID)
	ident := getIdentityForRequestSpecificToNode(s, action.Identity, action.NodeID)
	opt := options.WithIdentity(options.GetAllP2PDocuments(), ident)
