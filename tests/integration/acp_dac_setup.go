...
func setupSourceHub(s *state.State, testCase TestCase) ([]node.DocumentACPOpt, error) {
	...
	container, err := testcontainers.Run(ctx,
		...
		testcontainers.WithExposedPorts("1317/tcp"),
	...
	restEndpoint, err := container.PortEndpoint(ctx, "1317", "tcp")
	if err != nil {
		return nil, err
	}
	s.T.Logf("sourcehub endpoints: grpc=%v, rpc=%v, rest=%v", grpcEndpoint, rpcEndpoint, restEndpoint)
	...
	s.SourcehubRestAddress = restEndpoint
	s.SourcehubCometRPCAddress = rpcEndpoint
	s.SourcehubChainID = sourcehubTestChainID
	s.SourcehubSignerKey = privKeyBytes
