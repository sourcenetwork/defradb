...
type P2P interface {
	...
	Connect(ctx context.Context, addresses []string, opts ...*options.ConnectOptions) error
	...
	// subscribes to their topics. It will error if any of the provided
	// docIDs are invalid.
	CreateP2PDocuments(ctx context.Context, docIDs []string, opts ...*options.CreateP2PDocumentsOptions) error

	// DeleteP2PDocuments removes the given docIDs from the P2P system and
	// unsubscribes from their topics. It will error if the provided
	// docIDs are invalid.
	DeleteP2PDocuments(ctx context.Context, docIDs []string, opts ...*options.DeleteP2PDocumentsOptions) error

	// ListP2PDocuments returns the list of persisted docIDs that
	// the P2P system subscribes to.
	ListP2PDocuments(ctx context.Context, opts ...*options.ListP2PDocumentsOptions) ([]string, error)
