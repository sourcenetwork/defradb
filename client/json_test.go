...
func TestNewJSONAndMarshalJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:         "Int8",
			input:        int8(42),
			expected:     newJSONNumberInt(42, nil),
			expectedJSON: "42",
		},
		{
			name:         "Int16",
		...
		},
		{
			name:  "AnyArray",
			input: []any{"a", 1, true},
			expected: newJSONArray([]JSON{newJSONString("a", nil), newJSONNumberInt(1, nil),
		...
		},
		{
			name:  "Int8Array",
			input: []int8{1, 2, 3},
			expected: newJSONArray([]JSON{newJSONNumberInt(1, nil), newJSONNumberInt(2, nil),
				newJSONNumberInt(3, nil)}, nil),
			expectedJSON: "[1,2,3]",
		},
		{
			name:  "Int16Array",
