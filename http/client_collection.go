...
func (c *Collection) PurgeByDocIDs(
	...
	methodURL := c.http.apiURL.JoinPath("collections", c.Version().Name, "documents", "purge")

	req, err := http.NewRequestWithContext(ctx, http.MethodDelete, methodURL.String(), bytes.NewBuffer(bodyBytes))
	if err != nil {
		return err
	}
