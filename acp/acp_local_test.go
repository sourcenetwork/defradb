...
func Test_LocalACP_PersistentMemory_StartEmptyPath_Error(t *testing.T) {
	ctx := context.Background()
	var localACP ACPLocal

	localACP.Init(ctx, immutable.Some(""))
	err := localACP.Start(ctx)
	require.Error(t, err)
	require.ErrorIs(t, err, ErrStartingACPInEmptyPath)
}
...
func Test_LocalACP_InMemory_StartAndClose_NoError(t *testing.T) {
	...
	localACP.Init(ctx, immutable.None[string]())
	err := localACP.Start(ctx)
...
func Test_LocalACP_PersistentMemory_StartAndClose_NoError(t *testing.T) {
	...
	localACP.Init(ctx, immutable.Some(acpModulePath))
	err := localACP.Start(ctx)
	require.Nil(t, err)
