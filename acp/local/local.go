...
var _ acp.ACPSystemClient = (*LocalACP)(nil)

// LocalACP represents a local to the node implementation that makes no remote calls.
...
// NewLocalACP creates a new LocalACP with the given storeName and pathToStore.
// Returns an error if specified [pathToStore] with an empty [storeName].
func NewLocalACP(pathToStore string, storeName string) (*LocalACP, error) {
	localACP := LocalACP{
		pathToStore: immutable.None[string](),
		closed:      true,
	}

	if pathToStore != "" {
		if storeName == "" {
			return nil, acp.ErrLocalACPStoreNameEmptyWithPersistentStorage
		}
		fullPath := filepath.Join(pathToStore, storeName)
		localACP.pathToStore = immutable.Some(fullPath)
	}

	return &localACP, nil
}

func (l *LocalACP) Start(ctx context.Context) error {
	if !l.closed {
		return acp.ErrLocalACPAlreadyStarted
	}
	...
	if !l.pathToStore.HasValue() { // Use a non-persistent, i.e. in memory store.
		...
		opts = append(opts, runtime.WithPersistentKV(storeLocation))
	}
...
func (l *LocalACP) ResetState(ctx context.Context) error {
	err := l.Close()
	if err != nil {
		return err
	}
...
func (l *LocalACP) RegisterObject(
	ctx context.Context,
...
func (l *LocalACP) VerifyAccessRequest(
	ctx context.Context,
...
func (l *LocalACP) AddActorRelationship(
	ctx context.Context,
	policyID string,
	resourceName string,
	objectID string,
...
func (l *LocalACP) DeleteActorRelationship(
	ctx context.Context,
	policyID string,
	resourceName string,
	objectID string,
