...
type ACPModule interface {
	...
	// persistent data will be stored for the acp module.
	//
	// If the path does not have a value, then acp module will run in memory.
	Init(ctx context.Context, path immutable.Option[string])
	...
	//
	// If the path does not have a value, then acp module will run in memory.
	// If the path has a value, but the value is empty then returns an error.
	...
	//
	// Learn more about the DefraDB Policy Interface [DPI](/acp/ACP.md)
