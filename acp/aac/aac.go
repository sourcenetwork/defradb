...
const adminACPStoreName = "admin_acp"
...
func NewAdminACP() AdminACP {
	return AdminACP{}
}

var _ acp.ACPSystemClient = (*AdminACP)(nil)

func (a *AdminACP) Init(ctx context.Context, path string) {
	if path == "" {
		a.pathToStore = immutable.None[string]()
	} else {
		a.pathToStore = immutable.Some(path)
	}
}

func (a *AdminACP) Start(ctx context.Context) error {
	...
	a.closed = false
	if !a.pathToStore.HasValue() { // Use a non-persistent, i.e. in memory store.
		storeLocation = "in-memory"
		opts = append(opts, runtime.WithMemKV())
	} else { // Use peristent storage.
		storeLocation = a.pathToStore.Value()
		acpStorePath := storeLocation + "/" + adminACPStoreName
		opts = append(opts, runtime.WithPersistentKV(acpStorePath))
	}

	manager, err = runtime.NewRuntimeManager(opts...)
	if err != nil {
		return acp.NewErrInitializationOfACPFailed(err, "Admin", storeLocation)
	}

	a.manager = manager
	a.engine = services.NewACPEngine(manager)
	return nil
}

func (a *AdminACP) Close() error {
	if !a.closed {
		if a.manager != nil {
			err := a.manager.Terminate()
			if err != nil {
				return err
			}
		}
...
func (a *AdminACP) ResetState(ctx context.Context) error {
	err := a.Close()
	if err != nil {
		return err
	}
	...
	if a.pathToStore.HasValue() {
		storeLocation := a.pathToStore.Value()
		path := storeLocation + "/" + adminACPStoreName
...
func (a *AdminACP) AddPolicy(
	...
	response, err := a.engine.CreatePolicy(ctx, &createPolicy)
	if err != nil {
		return "", err
	}
...
func (a *AdminACP) Policy(
	...
	response, err := a.engine.GetPolicy(ctx, &request)
...
func (a *AdminACP) RegisterObject(
	ctx context.Context,
	...
	_, err = a.engine.RegisterObject(ctx, &req)
	return err
}

func (a *AdminACP) ObjectOwner(
	ctx context.Context,
	policyID string,
	resourceName string,
	objectID string,
	...
	result, err := a.engine.GetObjectRegistration(ctx, &req)
	if err != nil {
		return none, err
	}
...
func (a *AdminACP) VerifyAccessRequest(
	ctx context.Context,
	...
	resp, err := a.engine.VerifyAccessRequest(ctx, &req)
...
func (a *AdminACP) ValidateResourceInterface(
	...
	return acp.ValidateResourceInterfaceAccordingToACPSystem(
		ctx,
		policyID,
		resourceName,
		acpTypes.AdminACP,
...
func (a *AdminACP) AddActorRelationship(
	ctx context.Context,
	policyID string,
	resourceName string,
	objectID string,
	...
	setRelationshipResponse, err := a.engine.SetRelationship(ctx, &setRelationshipRequest)
	if err != nil {
		return false, err
	}
...
func (a *AdminACP) DeleteActorRelationship(
	ctx context.Context,
	policyID string,
	resourceName string,
	objectID string,
	...
	deleteRelationshipResponse, err := a.engine.DeleteRelationship(ctx, &deleteRelationshipRequest)
	if err != nil {
		return false, err
	}
