...
// Package planner creates DefraDB GraphQL Request (Top-level Query) Plans.
//
// Note: We use the word "request" to refer to the top-level query made to the database,
//       to avoid confusion with the query operation.
//
// DefraDB Request Planner
// =====================
//
// The DefraDB Request Planner creates an Execution Plan from the currently defined schemas,
// collections, request / query, and any possible indexes.
//
// An execution plan is a hierarchical structure that represents how and what data to
// request the database for. It defines what should be done at each execution step, and if it
...
// nodes may loop continuously until they can produce a result document. Eg. JoinNodes
...
// Next() method. Which will either return True or False, depending on if it successfully
...
// request query string, parsed into an appropriate AST Document. The AST Document
// is generated by the github.com/graphql-go/graphql package. It is then further
// parsed using a native DefraDB GraphQL Parser
// (github.com/sourcenetwork/defradb/request/graphql/parser), which converts the complex
// AST Document, into a manageable structure, with all the relevant query request information
// readily available.
//
// More details about the DefraDB Request (Top-level Query) Planner can be found in the DefraDB Technical
