...
func (mapping *DocumentMapping) Add(index int, name string) {
	appendToInner(mapping.IndexesByName, name, index)
...
func appendToInner(collection map[string][]int, key string, value int) {
	inner := collection[key]
	inner = append(inner, value)
	collection[key] = inner
}
