...
func (n *dagScanNode) Next() (bool, error) {
	...
	for {
		...
		if len(docIDs) == 0 {
			commit, err := n.dagBlockToNodeDoc(dagBlock, "", cols)
			if err != nil {
				return false, err
			}
			...
			for _, docID := range docIDs {
				commit, err := n.dagBlockToNodeDoc(dagBlock, docID, cols)
				if err != nil {
					return false, err
				}
				commits = append(commits, commit)
...
func (n *dagScanNode) dagBlockToNodeDoc(block *coreblock.Block, docID string, prefetchedCols []client.Collection) (core.Doc, error) {
