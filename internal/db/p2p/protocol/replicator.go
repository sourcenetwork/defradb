...
type pushLogProcessorFunc[Req message.Message] func(
	ctx context.Context,
	req Req,
	isReplicator bool,
) error

type replicatorFailureFunc[Req message.Message] func(ctx context.Context, peerID string, req Req) error

// ReplicatorProtocol is the protocol implementation for sending resource updates to a peer node.
type ReplicatorProtocol[Req message.Message, Rep message.Message] struct {
	*baseProto
	pushLogProcessorFunc  pushLogProcessorFunc[Req]
	replicatorFailureFunc replicatorFailureFunc[Req]
}

// NewReplicatorProtocol returns and a new [ReplicatorProtocol] struct and registers the protocol
// on the stream handler.
func NewReplicatorProtocol[Req message.Message, Rep message.Message](
	h client.Host,
	pushLogProcessorFunc pushLogProcessorFunc[Req],
	replicatorFailureFunc replicatorFailureFunc[Req],
) *ReplicatorProtocol[Req, Rep] {
	proto := &ReplicatorProtocol[Req, Rep]{
		baseProto:             newBaseProto(h),
		pushLogProcessorFunc:  pushLogProcessorFunc,
		replicatorFailureFunc: replicatorFailureFunc,
	}
...
//
// Callers should set an appropriate context timeout.
func (proto *ReplicatorProtocol[Req, Rep]) PushToReplicator(
	ctx context.Context,
	req Req,
	pid string,
	isRetry bool,
	...
	return message.Send[Rep](ctx, proto, req, pid, replicatorProtocolRequest)
}

func (proto *ReplicatorProtocol[Req, Rep]) onRequest(stream io.Reader, peerID string) {
	ctx := context.Background()
	var req Req
	err := message.Receive(stream, peerID, proto, req)
	if err != nil {
...
func (proto *ReplicatorProtocol[Req, Rep]) onResponse(stream io.Reader, peerID string) {
	var resp Rep
	_ = message.Receive(stream, peerID, proto, resp)
}
