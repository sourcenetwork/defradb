...
type AdminACPDesc struct {
	// IsEnabled is true if adminACP is turned on, and false otherwise.
	IsEnabled bool

	// IsClean is true if adminACP was never started yet, or was purged/reset, and false otherwise.
	IsClean bool
...
// NewCleanAdminACPDesc returns a new [AdminACPDesc] that represents a clean admin acp state.
func NewCleanAdminACPDesc() AdminACPDesc {
	return AdminACPDesc{
		IsEnabled: false,
		IsClean:   true,
...
func (db *DB) fetchAdminACPDesc(ctx context.Context, txn datastore.Txn) error {
	...
	storedAdminACPDesc := NewCleanAdminACPDesc()
...
func (db *DB) resetAdminACP(ctx context.Context) error {
	...
	db.adminInfo.AdminDesc = NewCleanAdminACPDesc()
	return nil
}
...
func (db *DB) cleanRegisterInternalAACPolicyAndGateNode(ctx context.Context) error {
	...
	policyID, err := db.adminInfo.AdminACP.AddPolicy(
		ctx,
		identity,
		acpTypes.InternalAdminACPPolicy,
	...
	err = db.adminInfo.AdminACP.ValidateResourceInterface(
		ctx,
		policyID,
		acpTypes.InternalAdminACPPolicyResourceName,
	)
	...
	err = db.adminInfo.AdminACP.RegisterObject(
		...
		policyID,
		acpTypes.InternalAdminACPPolicyResourceName,
		acpTypes.InternalAdminACPObjectToGate,
		protoTypes.TimestampNow(),
	)
	if err != nil { // We must fix the state before returning, as we already uploaded the policy.
		if errReset := db.resetAdminACP(ctx); errReset != nil {
	...
	policyDesc := client.PolicyDescription{
		ID:           policyID,
		ResourceName: acpTypes.InternalAdminACPPolicyResourceName,
	}

	db.adminInfo.AdminDesc.IsEnabled = true
	db.adminInfo.AdminDesc.IsClean = false
...
func (db *DB) ReEnableAAC(ctx context.Context) error {
	...
	if db.adminInfo.AdminDesc.IsClean {
		return ErrAACIsNotConfigured
	}

	if db.adminInfo.AdminDesc.IsEnabled {
...
func (db *DB) DisableAAC(ctx context.Context) error {
	...
	db.adminInfo.AdminDesc.IsEnabled = false
	return db.saveAdminACPDesc(ctx)
}
...
func (db *DB) checkAdminAccess(
	...
	if permissionNeeded.IsForAACOperation() && db.adminInfo.AdminDesc.IsClean &&
		permissionNeeded != acpTypes.AdminAACStatusPerm {
		return ErrAACIsNotConfigured
	}

	ident := identity.FromContext(ctx)
	if ident.HasValue() && db.nodeIdentity.HasValue() && ident.Value().DID == db.nodeIdentity.Value().DID {
		return nil
	}
...
func CheckAACNodeOperationAccess(
	...
	if !adminInfo.AdminDesc.IsEnabled && permission != acpTypes.AdminAACReEnablePerm {
	...
	if err != nil {
		return acp.NewErrFailedToVerifyAdminAccessWithACP(
			err,
			"AdminACP",
			permission.String(),
			policyID,
...
func (db *DB) addAACActorRelationship(
	...
	if !db.adminInfo.AdminDesc.IsEnabled || db.adminInfo.AdminACP == nil {
...
func (db *DB) AddAACActorRelationship(
	...
	err := db.checkAdminAccess(ctx, acpTypes.AdminAACRelationAddPerm)
	if err != nil {
		return client.AddActorRelationshipResult{}, err
	}
...
func (db *DB) DeleteAACActorRelationship(
	...
	err := db.checkAdminAccess(ctx, acpTypes.AdminAACRelationDeletePerm)
	if err != nil {
		return client.DeleteActorRelationshipResult{}, err
	}
...
func (db *DB) initializeAdminACP(ctx context.Context, txn datastore.Txn) error {
	...
	if db.adminInfo.AdminDesc.IsEnabled {
		if isAACEnabledInStartCmd {
			log.Info("Starting with aac (already setup & enabled, ignoring config re-enabling aac)")
			return nil
		}
...
func (db *DB) GetAACStatus(ctx context.Context) (client.StatusAACResult, error) {
	...
	err := db.checkAdminAccess(ctx, acpTypes.AdminAACStatusPerm)
	if err != nil {
		return client.StatusAACResult{}, err
	}

	return client.StatusAACResult{
		IsEnabled:    db.adminInfo.AdminDesc.IsEnabled,
		IsConfigured: !db.adminInfo.AdminDesc.IsClean,
	}, nil
}
