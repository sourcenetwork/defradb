...
func (db *DB) GetAllIndexes(
	...
	if err := db.checkAdminAccess(ctx, acpTypes.AdminIndexListPerm); err != nil {
		return nil, err
	}
...
func (db *DB) AddSchema(ctx context.Context, schemaString string) ([]client.CollectionVersion, error) {
	...
	if err := db.checkAdminAccess(ctx, acpTypes.AdminSchemaAddPerm); err != nil {
		return nil, err
	}
...
func (db *DB) SetActiveSchemaVersion(ctx context.Context, schemaVersionID string) error {
	...
	if err := db.checkAdminAccess(ctx, acpTypes.AdminSchemaSetActiveVersionPerm); err != nil {
		return err
	}
