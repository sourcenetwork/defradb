...
func trigramQueryFromFilter(conditions map[connor.FilterKey]any, fieldIndex int) *indextrigram.Query {
	q := indextrigram.AllQuery
...
func trigramQueryFromCondition(key connor.FilterKey, val any, fieldIndex int) *indextrigram.Query {
	switch k := key.(type) {
	case *mapper.PropertyIndex:
		if k.Index != fieldIndex {
			// A condition on another field says nothing about this one.
			return indextrigram.AllQuery
		}
	...
	case *mapper.Operator:
		...
		switch k.Operation {
		case opAnd:
			return foldTrigramBranches(branches, fieldIndex, indextrigram.AllQuery, (*indextrigram.Query).And)
		case opOr:
			return foldTrigramBranches(branches, fieldIndex, indextrigram.NoneQuery, (*indextrigram.Query).Or)
		}
	}
	return indextrigram.AllQuery
}
...
func foldTrigramBranches(
	branches []any,
	fieldIndex int,
	zero *indextrigram.Query,
	combine func(*indextrigram.Query, *indextrigram.Query) *indextrigram.Query,
) *indextrigram.Query {
	q := zero
...
func trigramQueryFromFieldOperators(condMap map[connor.FilterKey]any) *indextrigram.Query {
	q := indextrigram.AllQuery
	for key, val := range condMap {
		...
		switch op.Operation {
		case opLike, opILike:
			q = q.And(indextrigram.LikeQuery(pattern))
		case opRegex:
			q = q.And(indextrigram.RegexQuery(pattern))
		default:
			return indextrigram.AllQuery
		}
	}
...
func (iter *trigramIndexIterator) Init(ctx context.Context, store datastore.Keyedstore) error {
	...
	if iter.query.Op == indextrigram.QueryNone {
		return nil
	}
...
func (iter *trigramIndexIterator) newCursor(
	...
	q *indextrigram.Query,
) (trigramCursor, error) {
	subs := make([]trigramCursor, 0, len(q.Trigrams)+len(q.Sub))
	for _, trigram := range q.Trigrams {
		key := iter.baseKey
	...
	// trigramAll and indextrigram.QueryNone are handled by the caller and never reach a sub-node, since
	// andOr absorbs them rather than nesting them.
	if q.Op == indextrigram.QueryOr {
