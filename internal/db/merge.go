...
func (mp *mergeProcessor) processBlock(
	...
	if canRead {
		crdtInst, err := mp.initCRDTForType(ctx, dagBlock.Delta)
		if err != nil {
			return ctx, err
		}
		...
		err = coreblock.ProcessBlock(ctx, crdtInst, block, blockLink)
		if err != nil {
			return ctx, err
		}

		// Collect field values for new documents so we can build the doc for index sync
		// without needing to read it back from the store.
		if crdt.IsNewDocCreateMode(ctx) && !block.Delta.IsComposite() && !block.Delta.IsCollection() {
