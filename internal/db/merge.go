...
//
// A chunk that fails for anything other than a transaction conflict is re-run one
// event at a time so that an unmergeable event drops only itself. Dropped events are
