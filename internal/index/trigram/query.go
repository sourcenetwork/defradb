...
// Portions of this file are derived from Google Code Search.
// Copyright (c) 2011 The Go Authors. All rights reserved.
// See LICENSE.codesearch in this directory for the BSD 3-Clause license.
...
// Query is a boolean query over trigrams.
type Query struct {
	Op       QueryOp
	Trigrams []string
	Sub      []*Query
...
const (
	// QueryAll matches every document: the condition yielded nothing the index can use,
	// so the caller should scan instead.
	QueryAll QueryOp = iota
	// QueryNone matches no document at all.
	QueryNone
	// QueryAnd requires every trigram and sub-query to match.
	QueryAnd
	// QueryOr requires at least one trigram or sub-query to match.
	QueryOr
)

var AllQuery = &Query{Op: QueryAll}
var NoneQuery = &Query{Op: QueryNone}

// IsEvaluable reports whether q can be executed as a finite candidate-set expression. QueryAll and
// any unconstrained sub-expression require a full scan.
func (q *Query) IsEvaluable() bool {
	if q.Op == QueryNone {
		return true
	}
	if len(q.Trigrams) == 0 && len(q.Sub) == 0 {
		return false
	}
...
// LikeQuery returns the trigram query for a _like or _ilike pattern.
//
...
func LikeQuery(pattern string) *Query {
	q := AllQuery
...
// RegexQuery returns the trigram query for a _regex pattern.
//
// A pattern that does not parse yields QueryAll, so the query falls back to a scan and the
// filter reports the invalid pattern itself, exactly as it does with no index present.
func RegexQuery(pattern string) *Query {
...
func (q *Query) andOr(r *Query, op QueryOp) *Query {
	if len(q.Trigrams) == 0 && len(q.Sub) == 1 {
		q = q.Sub[0]
	}
	if len(r.Trigrams) == 0 && len(r.Sub) == 1 {
		r = r.Sub[0]
	}
	...
	qAtom := len(q.Trigrams) == 1 && len(q.Sub) == 0
	rAtom := len(r.Trigrams) == 1 && len(r.Sub) == 0
	if q.Op == op && (r.Op == op || rAtom) {
		q.Trigrams = stringSet.union(q.Trigrams, r.Trigrams, false)
		q.Sub = append(q.Sub, r.Sub...)
		return q
	}
	if r.Op == op && qAtom {
		r.Trigrams = stringSet.union(r.Trigrams, q.Trigrams, false)
		return r
	}
	if qAtom && rAtom {
		q.Op = op
		q.Trigrams = append(q.Trigrams, r.Trigrams...)
		return q
	}
...
// answers the two cases that keep QueryAll and QueryNone from being nested inside a
// query: a nested QueryAll node constrains nothing, and isEvaluable rejects it.
func (q *Query) implies(r *Query) bool {
	if q.Op == QueryNone || r.Op == QueryAll {
...
func (q *Query) andTrigrams(t stringSet) *Query {
	...
	for _, tt := range t {
		...
		trig.clean(false)
		or = or.Or(&Query{Op: QueryAnd, Trigrams: trig})
	}
	return q.And(or)
}
...
func (q *Query) String() string {
	...
	if len(q.Sub) == 0 && len(q.Trigrams) == 1 {
		return strconv.Quote(q.Trigrams[0])
	}
