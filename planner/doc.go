...
The DefraDB Query Planner creates an Execution Plan from the currently defined schemas, collections, query, and any
possible indexes.

An execution plan is a hierarchical structure that represents how and what data to query the database for. It defines
...
The plan starts with a base ast.Document, which represents the entire provided request query string, parsed into an
...
(https://github.com/sourcenetwork/defradb/query/graphql/parser), which converts the complex AST Document, into a
manageable structure, with all the relevant query information readily available.

More details about the DefraDB Query Planner can be found in the DefraDB Technical Specification Document.
*/
package planner
